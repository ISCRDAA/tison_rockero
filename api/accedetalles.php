...
if ($id == '' || $token == '') {
    ...
    if ($token == $token_tmp) {
        ...
        $sql = $conn->prepare("SELECT count(id)  FROM  accesorios WHERE id=? and activo = 1");
        $sql->execute([$id]);

        if ($sql->fetchColumn() > 0) {

            $sql = $conn->prepare("SELECT  nombre, descripcion, precio, descuento  FROM  accesorios WHERE id=? and activo = 1 LIMIT 1");
            $sql->execute([$id]);
            ...
            $rutaImg = $dir_images . 'accesorio.jpg';
    ...
    <header class="header">
        <section class="flex">
            ...
            <nav class="navbar">
                ...
                <a href="accesorios.php"> Regresar a los productos</a>
