...
if ($id == '' || $token == '') {
    ...
    if ($token == $token_tmp) {
        ...
        $sql = $conn->prepare("SELECT count(id)  FROM  tenis WHERE id=? and activo = 1");
        $sql->execute([$id]);

        if ($sql->fetchColumn() > 0) {

            $sql = $conn->prepare("SELECT  nombre, descripcion, precio, descuento  FROM  tenis WHERE id=? and activo = 1 LIMIT 1");
            $sql->execute([$id]);
    ...
    <header class="header">
        <section class="flex">
            ...
            <nav class="navbar">
                ...
                <a href="tenis.php"> Regresar a los productos</a>
